...
export default function AddHabitModal({ onClose }: Props) {
  ...
  const handleSave = () => {
    if (!name.trim()) return;
    addHabit({ name: name.trim(), icon, color, category, targetValue, targetUnit, schedule, reminderTime: reminderTime || null, pauseRanges: [] });
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end lg:items-center justify-center">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <span className="text-sm font-semibold tracking-widest" style={{ color: 'var(--color-ink-muted)', fontFamily: 'var(--font-sans)' }}>NEW HABIT</span>
          <button
            onClick={handleSave}
            disabled={!name.trim()}
            ...
            {COLORS.map(c => (
              <button
                ...
                style={{
                  ...
                  outline: color === c ? `3px solid var(--color-${c})` : 'none',
                  outlineOffset: 2,
                }}
                aria-label={c}
              />
        ...
        <div className="mb-5">
          ...
          <div className="flex items-center gap-2 p-4 rounded-2xl" style={{ background: 'var(--color-bg-soft)' }}>
            <input
              ...
              className="w-16 text-center font-mono font-medium text-xl bg-transparent outline-none"
          ...
          <div className="flex gap-2">
            {DAYS.map((d, i) => (
              <button
                ...
                style={{
                  background: schedule.includes(i) ? 'var(--color-forest)' : 'var(--color-bg-soft)',
                  color: schedule.includes(i) ? 'white' : 'var(--color-ink-muted)',
                  fontFamily: 'var(--font-sans)',
                }}
              >
                {d}
        ...
        <div className="mb-5">
          ...
          <div className="flex items-center gap-3 p-4 rounded-2xl" style={{ background: 'var(--color-bg-soft)' }}>
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path d="M8 2a6 6 0 100 12A6 6 0 008 2zM8 5v3.5l2.5 1.5" stroke="var(--color-ink-muted)" strokeWidth={1.5} strokeLinecap="round" />
            </svg>
            <input
              type="time"
              value={reminderTime}
          ...
          <div className="flex gap-2 flex-wrap">
            {CATEGORIES.map(cat => (
              <button
                ...
                style={{
                  background: category === cat ? 'var(--color-forest)' : 'var(--color-bg-soft)',
                  color: category === cat ? 'white' : 'var(--color-ink-muted)',
                  fontFamily: 'var(--font-sans)',
                }}
              >
                {cat}
